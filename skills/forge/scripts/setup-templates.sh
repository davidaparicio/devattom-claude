#!/bin/bash
# Forge — Initialise la structure de sortie depuis les templates
#
...
if [[ -z "$INPUT_NAME" ]] || [[ -z "$TASK_DESCRIPTION" ]]; then
    echo "Error: task-id et description requis" >&2
    exit 1
fi
...
# Parser input : task_id complet ou feature name seul
...
# Déterminer les statuts conditionnels
test_status="⏭ Skip"
...
# Construire le contenu des docs de référence
reference_docs_content="_Aucun document de référence._"
if [[ -n "$REFERENCE_FILE" ]]; then
    reference_docs_content="**À LIRE avant planification/exécution :** \`${REFERENCE_FILE}\`"
fi
...
# Toujours créer les fichiers de base
...
echo "✓ Forge templates initialisés : ${OUTPUT_DIR}"
exit 0
